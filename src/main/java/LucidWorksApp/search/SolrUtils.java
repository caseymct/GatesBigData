...
public class SolrUtils extends Utils {
    ...
    private static DateMathParser p = new DateMathParser(TimeZone.getTimeZone("UTC"), Locale.US);
    ...
    public static List<String> getSolrIndexDateRange(String collectionName, String dateField) {
        String url = SERVER + SOLR_ENDPOINT + "/" + collectionName + "/select";
        String urlParams = "?q=*:*&rows=1&wt=json&fl=" + dateField + "&sort=" + dateField;
        ...
        try {
            Long ms = (DateField.parseDate(dateRange.get(1)).getTime() - DateField.parseDate(dateRange.get(0)).getTime())/10;
    ...
    * */
    private static HashMap<String, String> retrieveFields(String collectionName) {
        ...
        for(String field : getFieldNameSubset(((JSONObject) json.get("fields")).names())) {
    ...
    public static void solrSearch(String collectionName, String queryString, String sortType, String sortOrder,
        ...
        HashMap<String, String> fields = retrieveFields(collectionName);
        ...
        try {
            ...
            for (Map.Entry field : fields.entrySet()) {
                if (field.getValue().equals("date")) {
                    List<String> dateRange = getSolrIndexDateRange(collectionName, (String) field.getKey());
